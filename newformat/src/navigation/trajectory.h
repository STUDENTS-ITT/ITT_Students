...
//   - makeRecord — строка: БИНС + эталон + ошибки Калмана
...
// Одна строка выходного файла: решение БИНС, эталон СНС.
inline data_io::NavRecord makeRecord(double time, const NavState &st, const SnsSample &ref)
{
    ...
    r.lon_sns = ref.lon * RAD_TO_DEG;
    r.lat_sns = ref.lat * RAD_TO_DEG;
    r.alt_sns = ref.alt;
    r.heading_sns = ref.heading * RAD_TO_DEG;
    r.pitch_sns = ref.pitch * RAD_TO_DEG;
    r.roll_sns = ref.roll * RAD_TO_DEG;
    r.vn_sns = ref.vn;
    r.vh_sns = ref.vh;
    r.ve_sns = ref.ve;
    return r;
                 ...
                 bool has_heading_ref)
{
    ...
    if (do_correction)
    {
        ...
        // Курс корректируется только при наличии angle.data (эталона курса).
    ...
    log.write(makeRecord(time_s, st, ref), x_out, st.lat);
}
