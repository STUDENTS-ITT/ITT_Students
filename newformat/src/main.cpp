...
//      результатов (ИМУ-такт) в kalman15_line2.txt, коррекция pitch/roll по акселю,
...
int main(int argc, char **argv)
{
    ...
    const std::string out_file = (tools_dir / "kalman15_line2.txt").string();

    const std::string imu_file = data_dir + "/imu.data";
    const std::string gps_file = data_dir + "/gps.data";
    const std::string angle_file = data_dir + "/angle.data";
    const std::string startup_file = data_dir + "/StartupNav.ini";

    const bool has_angle = std::filesystem::exists(angle_file);
    std::cout << "angle.data: " << (has_angle ? "found" : "not found (angles will be zero)") << std::endl;
