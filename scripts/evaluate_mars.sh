...
GT="${EVAL_GT:-${2:-$DATA/mars/mars_hkairport03_gt.tum}}"
[[ "$GT" == /* ]] || GT="$VNAV_ROOT/$GT"
...
if [[ -f "$VINS_CSV" ]]; then
    ...
    evo_rpe tum "$GT" vins.tum -a -r trans_part --delta 10 --delta_unit m --t_max_diff 0.05 \
        --no_warnings --save_results vins_rpe.zip | tee vins_rpe.txt
else
...
if [[ -f "$DSO_TUM" && -s "$DSO_TUM" ]]; then
    ...
    echo "ATE (Sim3):"
    evo_ape tum "$GT" dso.tum -as --t_max_diff 0.05 --no_warnings \
    ...
    evo_rpe tum "$GT" dso.tum -as -r trans_part --delta 10 --delta_unit m --t_max_diff 0.05 \
        --no_warnings --save_results dso_rpe.zip | tee dso_rpe.txt
else
...
if [[ -f vins.tum && -f dso.tum ]]; then
    ...
    evo_traj tum vins.tum dso.tum --ref="$GT" -a --plot_mode xy --no_warnings \
        --save_plot trajectories.pdf >/dev/null || echo "график evo_traj не сохранён"
elif [[ -f vins.tum ]]; then
    evo_traj tum vins.tum --ref="$GT" -a --plot_mode xy --no_warnings \
        --save_plot trajectories.pdf >/dev/null || echo "график evo_traj не сохранён"
fi
