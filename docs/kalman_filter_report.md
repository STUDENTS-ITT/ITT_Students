...
акселерометру**: она выполняется на каждом такте IMU и в нашей реализации является
...
| 2 | Автономная выставка (332 с) | `imu.dat` |
...
**Важно:** контур по акселерометру срабатывает **400 раз в секунду**; контур по СНС —
только при появлении нового кадра GPS. Именно поэтому наклон держится акселерометром,
...
- начальное смещение акселерометра **ba**.
...
Курс определяют по гироскопу — из проекций угловой скорости Земли на горизонт (после
оценки θ и φ):
...
что показал акселерометр, и тем, что *должна* давать гравитация при найденной ориентации:
...
### 3.1. Акселерометр задаёт абсолютную опору для наклона
...
### 3.2. Гироскоп на выставке нужен для курса, не для наклона
...
Сначала из сырых данных IMU получают ускорение в ССК тела с вычтенным ba
...
При выполнении gating доверие к акселерометру вычисляется плавно:
...
доверяет акселерометру. На границе допусков trust не падает ниже 0,05 — коррекция
...
**Выставка.** Смещение акселерометра ba оценивается обязательно: без него ломаются
